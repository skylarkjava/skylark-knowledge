...
public class ConfigControl extends Control {
	...
	public Boundary system() {
		...
		config = dao.selectOnKey(SystemConfig.SYSTEM_NAME_EXTERNAL, AppConfig.get().getSystemName());
		if (config == null) {
			config = new SystemConfigsEntity(SystemConfig.SYSTEM_NAME_EXTERNAL, AppConfig.get().getSystemName());
			config.setConfigValue("");
			dao.save(config);
		}
		setAttribute("systemtext", config.getConfigValue());
		SystemConfigLogic.get().setSystemText(config.getConfigValue());

		config = dao.selectOnKey(SystemConfig.SYSTEM_ENTRY_TEXT, AppConfig.get().getSystemName());
		if (config == null) {
			config = new SystemConfigsEntity(SystemConfig.SYSTEM_ENTRY_TEXT, AppConfig.get().getSystemName());
			config.setConfigValue("");
			dao.save(config);
		}
		setAttribute("entrytext", config.getConfigValue());
		SystemConfigLogic.get().setEntryText(config.getConfigValue());

		config = dao.selectOnKey(SystemConfig.SYSTEM_SLOGAN_TEXT, AppConfig.get().getSystemName());
		if (config == null) {
			config = new SystemConfigsEntity(SystemConfig.SYSTEM_SLOGAN_TEXT, AppConfig.get().getSystemName());
			config.setConfigValue("");
			dao.save(config);
		}
		setAttribute("slogantext", config.getConfigValue());
		SystemConfigLogic.get().setSloganText(config.getConfigValue());
	...
	public Boundary save_params() {
		...
		String systemtext = getParam("systemtext");
		String entrytext = getParam("entrytext");
		String slogantext = getParam("slogantext");
		...
		config = new SystemConfigsEntity(SystemConfig.SYSTEM_NAME_EXTERNAL, AppConfig.get().getSystemName());
		config.setConfigValue(systemtext);
		dao.save(config);
		SystemConfigLogic.get().setSystemText(systemtext);

		config = new SystemConfigsEntity(SystemConfig.SYSTEM_ENTRY_TEXT, AppConfig.get().getSystemName());
		config.setConfigValue(entrytext);
		dao.save(config);

		config = new SystemConfigsEntity(SystemConfig.SYSTEM_SLOGAN_TEXT, AppConfig.get().getSystemName());
		config.setConfigValue(slogantext);
		dao.save(config);
