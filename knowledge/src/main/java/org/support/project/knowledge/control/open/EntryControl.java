...
public class EntryControl extends KnowledgeControlBase {
	...
	public Boundary view() throws InvalidParamException, ParseException {
		...
		if (loginedUser == null) {
			...
			if (!entity.getPublicFlag().equals(KnowledgeLogic.PUBLIC_FLAG_PUBLIC)) {
				return super.redirect(getRequest().getContextPath() + "/protect.entry/view/" + knowledgeId);
			}
		}
	...
	public Boundary list() throws Exception {
		...
		String s1 = getCookie(SystemConfig.COOKIE_KEY_CATEGORY);
		Integer category = null;
		if (StringUtils.isInteger(s1)) {
			category = new Integer(s1);
			setAttribute("categoryId", category);
		}else {
			setAttribute("categoryId", "all");
		}
		...
		if (StringUtils.isInteger(tag)) {
			...
			knowledges.addAll(knowledgeLogic.showKnowledgeOnTag(category,tag, loginedUser, offset * PAGE_LIMIT, PAGE_LIMIT));
			...
			knowledges.addAll(knowledgeLogic.showKnowledgeOnGroup(category,group, loginedUser, offset * PAGE_LIMIT, PAGE_LIMIT));
			...
			knowledges.addAll(knowledgeLogic.searchKnowledge(category,keyword, tags, groups, loginedUser, offset * PAGE_LIMIT, PAGE_LIMIT));
		} else {
			// その他(キーワード検索)
			LOG.trace("search");
		...
		if (loginedUser != null && loginedUser.isAdmin()) {
			// 管理者であれば、ナレッジの件数は、参照権限を考慮していない
	 		List<TagsEntity> tags = tagsDao.selectTagsWithCount(category,0, FAV_PAGE_LIMIT);
			...
			List<TagsEntity> tags = tagLogic.selectTagsWithCount(category,loginedUser, 0, FAV_PAGE_LIMIT);
			setAttribute("tags", tags);
	...
	public Boundary show_history() throws InvalidParamException {
		...
		List<KnowledgesEntity> histories = knowledgeLogic.getKnowledges(category,historyIds, loginedUser);
		LOG.trace("履歴取得完了");
