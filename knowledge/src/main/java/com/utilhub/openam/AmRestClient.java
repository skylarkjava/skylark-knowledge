 ...
 */
public class AmRestClient {

    private static Logger LOG = LoggerFactory.getLogger(AmRestClient.class);

    protected String OpenAMRealm = "/";
    protected String OpenAMService = "activemq";
    protected String OpenAMHostName = "passport.utilhub.dt.hudaokeji.com";
    protected String OpenAMPort = "80";
    protected String OpenAMMethod = "http";
    protected String OpenAMURLPrefix = "openam";
	 ...
	 */
    public String getAttributes(String token) {
        String rc = doInvoke(getAttributesUrl(token));
		return rc;
	}
    ...
    protected String getAttributesUrl(String token) {
        return String.format("%s/identity/attributes?subjectid=%s", getURLPrefix(), token);
    }
