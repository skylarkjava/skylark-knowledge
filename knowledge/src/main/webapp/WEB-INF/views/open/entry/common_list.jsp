...
<div class="col-sm-12 col-md-8 knowledge_list">
	...
	<c:forEach var="knowledge" items="${list_data}" varStatus="status">
		<div class="knowledge_item">
			<a href="<%= request.getContextPath()%>/open.entry/view/<%= jspUtil.out("knowledge.knowledgeId") %><%= jspUtil.out("params") %>">
				<div class="insert_info">
					<h4><%= jspUtil.out("knowledge.title", JspUtilEx.ESCAPE_CLEAR) %></h4>
					<div>
			...
			<div class="item-info">
				<a class="btn-link" href="<%= request.getContextPath() %>/open.entry/likes/<%= jspUtil.out("knowledge.knowledgeId") %><%= jspUtil.out("params") %>" >
					<i class="fa fa-thumbs-o-up" style="margin-left: 5px;"></i>&nbsp;× <span id="like_count"><%= jspUtil.out("knowledge.likeCount") %></span>
				</a>
				&nbsp;&nbsp;&nbsp;
				<a class="btn-link" href="<%= request.getContextPath() %>/open.entry/view/<%= jspUtil.out("knowledge.knowledgeId") %><%= jspUtil.out("params") %>#comments">
			...
			<div class="item_caption">
				...
				<%= jspUtil.out("knowledge.content", JspUtilEx.ESCAPE_CLEAR, 300) %>
				</p>
			</div>
			</c:if>
		</div>
