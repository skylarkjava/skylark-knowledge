...
<link rel="stylesheet" href="<%= request.getContextPath() %>/vendor/share/css/share.min.css" />
...
<script type="text/javascript" src="<%= request.getContextPath() %>/vendor/share/js/jquery.share.min.js"></script>
...
<%= systemText %>-<%= jspUtil.out("title", JspUtilEx.ESCAPE_CLEAR) %>
</c:param>

<c:param name="PARAM_CONTENT">
	<!-- category -->
	<c:import url="/WEB-INF/views/commons/category/category_list.jsp">
		<c:param name="PARAM_NOT_EDITABLE" value="true"/>
	</c:import>

	<div class="row" id="content_head">
		<div class="col-sm-8">
			<h4 class="title"><%= jspUtil.out("title", JspUtilEx.ESCAPE_CLEAR) %></h4>
			...
			<div class="insert_info">
				...
				<% String userLink = "<a href=\"" + request.getContextPath() + "/open.entry/list/0?user=" + jspUtil.out("updateUser") +"\">"
					+ jspUtil.out("updateUserName", JspUtilEx.ESCAPE_CLEAR) + "</a>"; %>
				<% String historyLink = "<a href=\"" + request.getContextPath() + "/open.entry/histories/" + jspUtil.out("knowledgeId") +"\">"
			...
			<c:if test="${!empty tagNames}">
				<div class="tag_list">
					...
					<c:forEach var="tagName" items="${tagNames.split(',')}">
						<a href="<%= request.getContextPath()%>/open.entry/list?tagNames=<%= jspUtil.out("tagName") %>">
				...
				<c:forEach var="target" items="${targets.get(knowledgeId)}">
					<c:choose>
						<c:when test="${targetLogic.isGroupLabel(target.value)}">
							...
							<a href="<%= request.getContextPath()%>/open.entry/list?group=<%= jspUtil.out("groupId") %>">
						...
						<c:when test="${targetLogic.isUserLabel(target.value)}">
							...
							<a href="<%= request.getContextPath()%>/open.entry/list?user=<%= jspUtil.out("userId") %>">
								<span class="tag label label-info"><%= jspUtil.out("target.label") %></span>
							</a>
						</c:when>
						<c:otherwise>
		...
		<div class="col-sm-4">
			<div class="article_buttons">
				...
				<% if (request.getRemoteUser() != null) {
					if ((boolean) request.getAttribute("edit")) { %>
						<a href="<%= request.getContextPath() %>/protect.entry/view_edit/<%= jspUtil.out("knowledgeId") %>"
				...
				<% } else { %>
					<a href="<%= request.getContextPath() %>/protect.entry/view/<%= jspUtil.out("knowledgeId") %>"
			...
			<div class="social-share"  data-description="<%= systemText %>-<%= jspUtil.out("title", JspUtilEx.ESCAPE_CLEAR) %>" data-title="<%= systemText %>-<%= jspUtil.out("title", JspUtilEx.ESCAPE_CLEAR) %>" data-disabled="diandian,google,twitter,facebook,linkedin" title="一键分享到微博，QQ，微信，腾讯微博，豆瓣，QQ空间"></div>

			<div class="article_info">
				<a href="<%= request.getContextPath() %>/open.entry/likes/<%= jspUtil.out("knowledgeId") %><%= jspUtil.out("params") %>" >
	...
	<div class="row" id="content_main">
		<div class="col-sm-12">
			...
			<%= jspUtil.out("content", JspUtilEx.ESCAPE_NONE) %>
			</div>
	...
	<% List commentList = jspUtil.getValue("comments", List.class);
		...
		<div class="row">
			...
			<% if (jspUtil.isAdmin() || jspUtil.is(jspUtil.id(), "comment.insertUser") || (boolean) request.getAttribute("edit")) { %>
				&nbsp;
				<a class="btn btn-primary btn-xs" href="<%= request.getContextPath() %>/protect.entry/edit_comment/<%= comment.getCommentNo() %>">
		...
		<%= jspUtil.out("comment.comment", JspUtilEx.ESCAPE_NONE) %>
			<!-- コメントに付けた添付ファイルの表示 -->
			<c:forEach var="file" items="${files}" >
...
<h4 class="sub_title"><i class="fa fa-comment-o"></i>&nbsp;<%= jspUtil.label("knowledge.comment.add") %></h4>
	<% if (request.getRemoteUser() != null) { %>
		<form action="<%= request.getContextPath()%>/protect.entry/comment/<%= jspUtil.out("knowledgeId") %><%= jspUtil.out("params") %>"
	...
	<% } else { %>
		<form action="<%= request.getContextPath()%>/protect.entry/view/<%= jspUtil.out("knowledgeId") %>" method="get" role="form">
