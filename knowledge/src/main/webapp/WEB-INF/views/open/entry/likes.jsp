...
<c:import url="/WEB-INF/views/commons/layout/layoutMain.jsp">
	...
	<c:param name="PARAM_CONTENT">
		...
		<nav>
			<ul class="pager">
				<li class="previous">
					<a href="<%= request.getContextPath() %>/open.entry/likes/<%= jspUtil.out("knowledgeId") %><%= jspUtil.out("params") %>&page=<%= jspUtil.out("previous") %>">
						<span aria-hidden="true">&larr;</span><%= jspUtil.label("label.previous") %>
					</a>
				</li>
				<li class="next">
					<a href="<%= request.getContextPath() %>/open.entry/likes/<%= jspUtil.out("knowledgeId") %><%= jspUtil.out("params") %>&page=<%= jspUtil.out("next") %>">
